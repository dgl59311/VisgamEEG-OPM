...
for nsubject in range(n_subjects):
    ...
    X_eeg = mne.Epochs(eegdata.copy().filter(l_freq=None, h_freq=None),
        event_id=eeg_events, tmin=-0.75, tmax=1,
        baseline=None, proj=False, detrend=1, reject_by_annotation=True, preload=True)
    X_opm = mne.Epochs(opmdata.copy().filter(l_freq=None, h_freq=None),
    ...
    for modality, X, datafolder_raw in [('eeg', X_eeg, datafoldereeg), ('opm', X_opm, datafolderopm)]:
        ...
        fmin_plot = 0    # matches the 5 Hz highpass already applied to X_eeg/X_opm
        ...
        np.savez(os.path.join(psds_dir, f'extended_baseline_post_psd_{modality}.npz'),
